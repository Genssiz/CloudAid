...
public class CloudAid {
	...
	public CloudAid(int mode, int scenario, int method, int algorithm){
		PrintWriter outAggregation;
		PrintWriter outSearch;
		try {
			outSearch = new PrintWriter("./Tests/SearchTests_"+System.currentTimeMillis()+".txt");
			outAggregation = new PrintWriter("./Tests/AggregationTests_"+System.currentTimeMillis()+".txt");
			outSearch.println(System.currentTimeMillis());
			outAggregation.println(System.currentTimeMillis());
			...
			this.searchEngine = new SearchCore(outSearch, scenario);
			...
			this.aggregatorEngine = new AggregationCore(outAggregation, scenario);
			
			this.mode = mode;
			this.init(method, scenario, algorithm);
			outSearch.close();
			outAggregation.close();
	...
	private void start(CSAData data, int methodID, int scenario, int algorithm){ 
		...
		if(scenario != DataSimulator.SEARCHTEST)
			...
			if(data.getEvalResult() == CSAEvaluator.OK){
				try {
					...
					for(ServiceTemplate c : comps){
						ArrayList<Offering> alternatives = this.search(c, scenario);
	...
	private ArrayList<Offering> search(ServiceTemplate c, int scenario){
	...
	public static void main(String[] args) {
		String[] scenarios = {"heroku", "usecase", "reqs", "norm", "search", "example"};
