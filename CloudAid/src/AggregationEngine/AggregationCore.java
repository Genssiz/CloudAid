...
public class AggregationCore {
	...
	public AggregationCore(PrintWriter out, int scenario){
	...
	public CSAData computeAggregation(CSAData data, int method, double incDelta, int algorithm){
		...
		if(this.scenario == DataSimulator.EXAMPLETEST){
			reps = 100;
		}
		for(int i = 1; i <= reps; i++){
			...
			result = new CSAData();
			admissables = null;
			admissables = new ArrayList<ArrayList<Result>>();
			solution = new ArrayList<Result>();
			out.println("RUN Nr "+i);
			
			if(this.scenario == DataSimulator.EXAMPLETEST){
				Random rand = new Random();

				float finalX = rand.nextFloat() * (5 - 1) + 1;
				...
				comb.setData(data);
				out.println("Price Limit Value: "+ finalX);
			}
			...
			out.println("Algorithm Time: "+ this.algorithmTime);
			out.println("Visited Nodes: "+ this.visitedNodes);
			out.println("Average Degree: "+ this.avgDegree);
			out.println("Average Depth: "+ this.avgDepth);
			out.println("ADMISSIBLES:");
			this.printResult(admissables, out);
	...
	public void printResult(ArrayList<ArrayList<Result>> result, PrintWriter out){
		...
		for(ArrayList<Result> list : result){
			...
			for(Result res : list){
				...
				totalPrice = totalPrice + Double.parseDouble((String) res.getService().getAttributes().get("price"));
			}
			out.println("");
			out.println("Total Aggregation Price: "+totalPrice);
		}
		out.println("--------------");
	}
			...
			for(Test t : testBatteries){
				...
				for(int d = 0; d < deltas.size(); d++){
					for(int i = 0; i < iterations; i++){						
						...
						AggregationCore aggreg = new AggregationCore(out, DataSimulator.NONE);
						CSAData result;
						if(d == 0){
